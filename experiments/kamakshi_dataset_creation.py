...
train_size = 0.7
max_len = 1200
np_data_out_dir = '/Users/thomasnuttall/code/DeepGRU/data/kamakshi/'
label_lookup = {i:s for i,s in enumerate(unique_svaras)}
...
np_data = list(zip(labels, np_data, static_features))

random.seed(42)
random.shuffle(np_data)

feat_data = np.array([x[2] for x in np_data])
np_data = [x[:2] for x in np_data]
#np_data_pad = []     
...
#np_data = np_data_pad
#np_data = np.c_[labels, np_data]

# Shuffle


train_data = np_data[:round(train_size*len(np_data))]
test_data = np_data[round(train_size*len(np_data)):]
train_feat_data = np.array(feat_data[:round(train_size*len(feat_data))])
test_feat_data = np.array(feat_data[round(train_size*len(feat_data)):])

# Write
train_path = cpath(np_data_out_dir, 'TRAIN.pkl')
train_feat_path = cpath(np_data_out_dir, 'TRAIN_FEAT.pkl')
test_path = cpath(np_data_out_dir, 'TEST.pkl')
test_feat_path = cpath(np_data_out_dir, 'TEST_FEAT.pkl')
label_path = cpath(np_data_out_dir, 'LABELS.pkl')
write_pkl(train_data, train_path)
write_pkl(train_feat_data, train_feat_path)
write_pkl(test_data, test_path)
write_pkl(test_feat_data, test_feat_path)
